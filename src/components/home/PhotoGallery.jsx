  ...
  return (
    <section id="gallery" className="py-24 bg-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        {/* 🖼️ Gallery Grid (No Filters) */}
        <motion.div
          layout
          className="grid md:grid-cols-2 lg:grid-cols-4 gap-6"
        >
          <AnimatePresence>
{(gallery.length > 0 ? gallery : projects).slice(0, 8).map((project, index) => (
              <motion.div
                key={project.id}
                layout
              ...
              >
                {/* Image */}
                  ...
                  <img src={project.image || floor} alt={project.title}
...
<div className="text-center mt-12">
  <Link
    to="/gallery"
    className="inline-block px-8 py-3 bg-gradient-to-r from-[#0b2343] to-[#008c94] text-white font-semibold rounded-lg shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300"
  >
    View More
  </Link>
</div>
